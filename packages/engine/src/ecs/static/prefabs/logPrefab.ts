...
/** Spawn recipe for a `log` def: the shared static component set, no extras. */
export function createLogPrefab(
  components: StaticComponents,
): Prefab<StaticPrefabParams> {
  return {
    name: 'log',
    build(world: World, params?: StaticPrefabParams): EntityId {
      return stampCommon(world, components, requireDef('log', params));
    },
  };
}
