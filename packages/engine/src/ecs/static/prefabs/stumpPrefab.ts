...
/** Spawn recipe for a `stump` def: the shared static component set, no extras. */
export function createStumpPrefab(
  components: StaticComponents,
): Prefab<StaticPrefabParams> {
  return {
    name: 'stump',
    build(world: World, params?: StaticPrefabParams): EntityId {
      return stampCommon(world, components, requireDef('stump', params));
    },
  };
}
